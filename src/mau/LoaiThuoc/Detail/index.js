...
function DetailLoaiThuoc(props) {
    ...
    const handleSubmit = async (values) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (loaithuoc?.ServiceCategoryID) {
                    handleUpdate(finalData);
                    dispatch(setDataLoaiThuoc());
                } else {
                    handleCreate(finalData);
    ...
    useEffect(() => {
        if (open) {  // Chỉ set giá trị khi modal mở
            form.setFieldsValue({
                ServiceCategoryID: loaithuoc?.ServiceCategoryID,
                ServiceGroupID: loaithuoc?.ServiceGroupID,
                ServiceCategoryName: loaithuoc?.ServiceCategoryName,
            });
        }
    }, [loaithuoc, open]);
                ...
                >
                    ...
                    >
                        {listServiceGroup?.map((item) => (
                            <Select.Option key={item.ServiceGroupID} value={item.ServiceGroupID}>
